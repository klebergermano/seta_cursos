  ...
  return new Promise((resolve, reject) => {
    ...
    connection.query("INSERT INTO carne SET ?", new_carne, (err) => {
      if (!err) {
        ...
        connection.query(
          "SELECT * FROM carne ORDER BY ID DESC LIMIT 1",
          (err, results) => {
            if (!err) {
              //-----------------------------------------------------------------------
              var id_carne = results[0].id;
              var id_contrato = req.body.carne.id_contrato;

              connection.query(
                "UPDATE contrato SET id_carne = ? WHERE id = ?",
                [id_carne, id_contrato],
                (err) => {
                  if (!err) {
                  } else {
                    console.log(err);
              ...
              for (let index in req.body.carne_folhas) {
                ...
                console.log(req.body.carne_folhas[index].id_carne);
